...
func (hash *HashM) SHA1File(filePath string) {
	...
	for {
		...
		if byteCount == 64 {
			for i := 0; i < 16; i++ {
				wordArray[i] = uint32(buffer[i*4])<<24 + uint32(buffer[i*4+1])<<16 + uint32(buffer[i*4+2])<<8 + uint32(buffer[i*4+3])
			}
			hash.sha1Round(wordArray)
		} else {
			buffer[byteCount] = 1 << 7
			byteCount += 1

			if byteCount <= 56 {
				for i := byteCount; i < 56; i++ {
					buffer[i] = 0
				}

				beSize := binary.BigEndian.AppendUint64(*new([]byte), uint64(fileSize)*8)

				for i := 56; i < 64; i++ {
					buffer[i] = beSize[i-56]
				}
